...
            buttonsContainer.appendChild(zoomIndicator);
            ...
            nameInput.id = `image-name-input-${pageNum}`;
            nameInput.value = `${originalFileName}_page_${pageNum}`;
            nameInput.title = '修改圖片檔名';
            buttonsContainer.appendChild(nameInput);
            // =========================
...
        ctx.drawImage(drawingCanvas, 0, 0);
        ...
        // === 修改：從輸入框獲取檔名 ===
        const nameInput = document.getElementById(`image-name-input-${pageNum}`);
        const fileName = (nameInput && nameInput.value) ? nameInput.value : `${originalFileName}_page_${pageNum}`;
        // =========================
